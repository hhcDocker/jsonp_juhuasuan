<template>
	<div class="main" style="padding-bottom: 50px;">
		...
		<!-- 商品信息 -->
		<div id="J_itemInfo" data-spm="iteminfo" v-if="baseInfo">
        ...
        <!-- 店铺详情 -->
        <div id="J_shopH5Desc" v-if="seller">
              <div class="shopdes">
                    <img class="shoppic" :src="'http:'+seller.shopIcon">
                    <div class="shopname">{{seller.sellerNick}}</div>
                    <img class="tm_icon" src="http://gtms04.alicdn.com/tps/i4/TB1ipsxIVXXXXauXXXXPDprHFXX-176-24.png">
                    <ul class="shopasess" >
                          <li v-for="item in seller.evaluates">{{item.title}}:<span class="red">{{item.score}}</span><span v-bind:class="['grade',item.level*1>=0?'good':'bad']">{{item.level | setLevel}}</span></li>
                    </ul>
              </div>
        </div>
        <!-- 宝贝描述，评价，参数tab -->
							...
							<div class="J_skuScroll">
								<div class="dp-cont">
									<div v-for="item in skuProps" class="dpc-p">
										<div class="dpc-tl">{{item.name}}</div>
										<div class="dpc-pr" propname="尺码">
											<span v-for="value in item.values" :data-img="value.image?value.image:''" class="J_skuspan" :data-id="item.pid+':'+value.vid" @click="togglebtn">{{value.name}}</span>
										</div>
									</div>
								</div>
								<div class="dp-num">
        ...
        <ul id="J_TabCon" class="tabcomwap">
            <li id="J_itemH5Desc" v-show="tabs == 0 "> 
                <div class="h5-desc">
                    <div id="J_pages" class="des-pages" v-if="imgInfo">
                        <img v-for="item in imgInfo"  v-if="item.params?item.params.picUrl?true:false:false" width="100%" class="des-alimg" v-lazy="item.params.picUrl">
                    </div>
                </div>
            </li>
            ...
            <li id="J_itemShopinfor" class="itemshopinfor" v-show="tabs == 2">
                <ul>
                    <li v-for="item in props"><p v-for="(value, key) in item"><span>{{key}}:  </span>{{value}}</p></li>
                </ul>
            </li>
        </ul>
        <!-- -->
            ...
            <div class="btnbar">
                <div class="btngroup col_2">
                    ...
                    <div id="J_BtnBuy" class="btn buy" @click="chooseByMethod('true',$event)">
                        <span class="subText" v-if="baseInfo">{{baseInfo.picFeature.sellingPoints[0]}}</span><span class="text">马上抢</span>
                    </div>
                </div>
            </div>
        </div>
	...
	export default{
        ...
        methods:{
            ...
            togglebtn(ev){
                ...
                $('.dpc-pr').forEach(function(item,index){
                    ...
                    if(sel.length){ 
                        ...
                        that.reg_currentSku.splice(that.order[index],1,sel.attr('data-id')+';?')                  
                    }else{
                       isContinue = false
                    }
                })
                ...
                console.log(that.reg_currentSku)
                if(isContinue){
                    window.kk = that.skuMap
                    for(var i in that.skuMap){
                       
                        if(eval('/'+that.reg_currentSku.join('')+'/').test(i)){
                    ...
                    that.limit = matchObj.limit || 5;
                    that.quantity = matchObj.quantity;
                    console.log('price: '+that.price +'   limit: '+that.limit +'   quantity: '+that.quantity)
            ...
            addToCart(ev){
                ...
                obj['imgUrl'] = 'gtd.alicdn.com/tps'+this.img;
		...
		created(){
            ...
            this.$http.jsonp('https://detail.ju.taobao.com/detail/json/mobile_dynamic.do'+location.search+'&groupId=&sales_site=1').then(function(response){
                var responseJson = response.body.item;
                this.$set(this,'baseInfo', response.body.item);

                this.price = responseJson.activityPrice.toString().slice(0,-2);
                this.limit = responseJson.limitNum;
                this.img = responseJson.picUrlNew;
                this.originalPrice = responseJson.originalPrice.toString().slice(0,-2);

                var Slider = require('../public/js/slider.js');
                var arr = [];
                
                this.$nextTick(() => {
                    if(responseJson.picUrl){
                        new Slider([{'src':'http://gtd.alicdn.com/tps'+responseJson.picUrl+'_600x600Q30.jpg_.webp'}])
                    }else{
                        responseJson.auxilaryPicList.forEach(function(item){
                            arr.push({'src':'http://gtd.alicdn.com/tps'+item+'_600x600Q30.jpg_.webp'})
                        })
                        new Slider(arr)
                    }                   
                })
            ...
            this.$http.jsonp('https://unszacs.m.taobao.com/h5/mtop.taobao.detail.getdetail/6.0/?appKey=12574478&t=1491482927080&sign=82b15b1bb3db9209455247126e8f66d8&api=mtop.taobao.detail.getdetail&v=6.0&type=jsonp&dataType=jsonp&ttid=2017%40tmall_h5_5.31.1&data=%7B%22itemNumId%22%3A%22'+location.search.match(/item_id=([0-9]+)/)[1]+'%22%7D').then(function(response){

                var responseJson = response.body.data;

                this.$set(this,'seller',responseJson.seller)
                //this.seller = responseJson.seller;//
                this.props = responseJson.props.groupProps[0]['基本信息'];//
                //console.log(this.seller)
                

                var apiStack = JSON.parse(responseJson.apiStack[0].value);
                var skuMap = {};
                
                this.skuProps = apiStack.skuBase.props;//
                this.skus = apiStack.skuCore.sku2info;

                apiStack.skuBase.skus.forEach(function(item,index){
                    skuMap[item.propPath] = item.skuId
                })
                console.log(that.reg_currentSku)

                this.skuMap = skuMap;//
                //console.log(this.skuMap)
                this.quantity = this.skus[0].quantity
                ...
                //确定组合skus的顺序
                var test = apiStack.skuBase.skus[0].propPath.match(/([0-9]+):/g);
                this.skuProps.forEach(function(item){
                    that.order.push(test.indexOf(item.pid+':'))
                    that.reg_currentSku.push('[0-9]+;?')
                })
                console.log(that.order)
            })

            this.$http.jsonp('https://detail.ju.taobao.com/detail/item_new_desc.json?'+location.search).then(function(response){
                this.imgInfo = response.data.model.children
            })

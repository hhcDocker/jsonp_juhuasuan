...
<div class="main" style="margin-top:.4rem;">
	...
	<div v-if="boxGroup.length" class=" box-group box-type-group " pre-display="block" style="overflow:hidden;display: -webkit-box;">
...
var Slider = require('../public/js/slider.js')
	export default{
		...
		mounted(){
            console.log($('.jx-julist')[0])

           /* setTimeout(function(){                
                console.log('firstH: '+document.body.offsetHeight)
            },1000)*/
        ...
        updated(){
            this.ulH = $('.jx-julist')[0].offsetHeight;
        },
        watch: {
            // 如果路由有变化，会再次执行该方法
            '$route': 'changeData',
        ...
        methods:{
            changeData(){
               ...
               // var data = require('../json/index_'+this.$route.params.id+'.js');
                ...
                this.$http.jsonp('https://ju.taobao.com/json/tg/ajaxGetItemsV2.json?&page='+this.page+'&psize=20&type=0&frontCatId='+this.$route.params.id).then(function(response){
            ...
            loadGoods(){
                ...
                this.$http.jsonp('https://ju.taobao.com/json/tg/ajaxGetItemsV2.json?&page='+this.page+'&psize=20&type=0&frontCatId='+this.$route.params.id).then(function(response){
                    ...
                    if(response.body.itemList.length<1){
                        that.isTotal = true;
                    }else{
